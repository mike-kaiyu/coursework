...
grid on;
axis([0 fs*Tb1*N1 -4.5 4.5]);
title('Bipolar signal in rectangular pulse shaping in baseband');

% example_mix=awgn(rect_pulse, 20); %example: adding noise which is 20dB. 
% figure;
% plot(t2, rect_pulse);%baseband signal waveform in time domain.
% hold on;
% plot(t2, example_mix);
% grid on;
% axis([0 fs*Tb1*N1 -3 3]);
% title('Bipolar signal with AWGN in rectangular pulse shaping in baseband');
...
tBER1=qfunc(sqrt((8./(10.^(noise_index./10))))); 
hold on;
semilogy(10*log10(8./(10.^(noise_index./10))),(tBER1)); 
grid on;
xlim([-10 10]);
ylim([0.0005 1]);
...
% ylabel('Magnitude (/dB)');
% %axis([-8 8 -30 30]);
% title('Power spectrum density of baseband bipolar signal in rectangular pulse');